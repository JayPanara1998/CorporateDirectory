...
const DirectoryHook: React.FC<IDirectoryProps> = (props) => {
    ...
    //                     // Group Name: SSO - HiBob - Users (Object Id: 2ab6104f-e5d6-4999-b3f2-7d688a87a850) - we are fetching the users from this Azure AD group. 
    //                     `https://graph.microsoft.com/v1.0/groups/{2ab6104f-e5d6-4999-b3f2-7d688a87a850}/members?$select=companyName,displayName,mail,officeLocation&$top=999`,
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const client = await props.context.aadHttpClientFactory.getClient("https://graph.microsoft.com");
    
                // Fetch users from the first group - "SSO - HiBob - Users" (Object Id: 2ab6104f-e5d6-4999-b3f2-7d688a87a850)
                const response1 = await client.get("https://graph.microsoft.com/v1.0/groups/{2ab6104f-e5d6-4999-b3f2-7d688a87a850}/members?$select=companyName,displayName,mail,officeLocation,accountEnabled&$top=999", AadHttpClient.configurations.v1);
                const json1 = await response1.json();
    
                // Fetch users from the second group - "SSO - Intranet - Consultants" (Object Id: a83181db-e15f-454d-a0e8-2cd6fb4d46a8)
                const response2 = await client.get("https://graph.microsoft.com/v1.0/groups/{a83181db-e15f-454d-a0e8-2cd6fb4d46a8}/members?$select=companyName,displayName,mail,officeLocation,accountEnabled&$top=999", AadHttpClient.configurations.v1);
                const json2 = await response2.json();
    
                // Combine the results into a single array
                const combinedUsers = [...json1.value, ...json2.value];
    
                // Process and set the combined users in userArray
                _apUserGraphArray(combinedUsers);
            } catch (error) {
                console.error(error);
            }
        };
